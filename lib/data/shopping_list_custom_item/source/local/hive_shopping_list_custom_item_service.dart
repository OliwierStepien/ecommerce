...
class HiveShoppingListCustomItemServiceImpl implements HiveShoppingListCustomItemService {
  ...
  Future<void> addCustomItemToShoppingList(ShoppingListCustomItemModel item) async {
    ...
    debugLog('✅ Dodano własny składnik: ${item.customItemName}', name: 'HiveService');
    debugLog('🛒 Liczba własnych składników: ${items.length}', name: 'HiveService');
  }
  ...
  Future<void> removeCustomItemFromShoppingList(String customItemId, {bool isOnline = false}) async {
    ...
    if (item != null) {
      if (isOnline) {
        ...
        debugLog('🗑️ Trwale usunięto zsynchronizowany składnik: $customItemId', name: 'HiveService');
      } else {
        ...
        debugLog('❌ Oznaczono jako usunięty: ${item.customItemName}', name: 'HiveService');
      }
    }
  }
  ...
  Future<void> markShoppingListCustomItemAsSynced(String customItemId) async {
    ...
    if (model != null) {
      ...
      debugLog('✅ Zaznaczono jako zsynchronizowany: ${model.customItemName}', name: 'HiveService');
    }
  }
  ...
  Future<void> restoreCustomItemToShoppingList(ShoppingListCustomItemModel item) async {
    ...
    debugLog('♻️ Przywrócono składnik: ${item.customItemName}', name: 'HiveService');
    debugLog('📦 Ilość wpisów w Hive: ${allItems.length}', name: 'HiveService');
    debugLog('🛒 Aktywne: ${activeItems.length}, 🗑️ Usunięte: ${deletedItems.length}', name: 'HiveService');
  }
  ...
  Future<void> clearSyncedDeletedItems() async {
    ...
    for (final key in keysToDelete) {
      ...
      debugLog('🧹 Usunięto trwale zsynchronizowany składnik: $key', name: 'HiveService');
    }
  }
}